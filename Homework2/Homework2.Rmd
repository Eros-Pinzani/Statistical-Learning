...
subtitle: "Codice annotato per la presentazione — tre slide"
...
# Slide 1 — Bias, Varianza e curva MISE

## Obiettivo

La prima slide è **teorica**: mostra visivamente perché esiste un $h$ ottimale, disegnando
...
B     <- 1000                              # numero di campioni Monte Carlo
h_seq <- seq(0.05, 2.5, length.out = 100) # griglia di h
...
for (b in seq_len(B)) {
  ...
  for (j in seq_along(h_seq)) {
    ...
    fhat <- density(samp, bw = h_seq[j],
                    ...
                    n    = length(x_grid))$y
    ise_mat[b, j]  <- sum((fhat - p_true_vec)^2) * dx   # integrazione numerica corretta
...
```{r slide1-grafico, fig.height=4.2}
df_curve <- rbind(
...
lwd_curve <- c("Bias\U00B2" = 0.9, "Varianza" = 0.9, "MISE" = 1.4)
...
```{r slide2-tre-pannelli, fig.height=3.2}
p_small <- panel_kde(
  h       = 0.10,
...
```{r slide2-opt-panel, fig.height=3.2}
p_opt_panel
```

```{r slide2-large-panel, fig.height=3.2}
p_large
```

## Layout combinato (4 pannelli — pronto per la slide)

```{r slide2-layout-completo, fig.height=7.5, fig.width=10}
layout_slide2 <- (p_dgp) /
...
# Slide 3 — Confronto selettori di $h$ e curva ISE

## Obiettivo

La terza slide ha **due pannelli affiancati**:

- **Sinistra** — tabella con i valori di $h$ prodotti da cinque metodi automatici **più
  $h^*$ (MISE empirico)** come riferimento ottimale, con il relativo MISE (calcolabile
  qui perché conosciamo $f$).
- **Destra** — curva MISE$(h)$ con una linea verticale colorata per ciascun selettore
...
suggerito — interpolando sulla griglia `h_seq` già simulata nella Slide 1.
...
Il grafico mostra il **MISE empirico** (stimato con gli stessi $B$ campioni della Slide 1)
...
```{r slide3-curva-ise, fig.height=4.5}
...
p_ise <- ggplot(df_mise_plot, aes(x = h, y = MISE)) +
  ...
  scale_x_continuous(limits = c(0, max(df_vlines$h, na.rm = TRUE) * 1.3)) +
