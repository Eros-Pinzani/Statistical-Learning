...
# Sezione 1 — Bias, Varianza e curva MISE

## Obiettivo

La prima sezione è **teorica**: mostra visivamente perché esiste un $h$ ottimale, disegnando
...
# Sezione 2 — DGP e variazione di $h$

## Obiettivo

La seconda sezione ha **quattro pannelli**:
...
```{r sez2-tre-pannelli, fig.height=3.2}
p_small <- panel_kde(
  h       = 0.01,
...
```{r sez2-opt-panel, fig.height=3.2}
p_opt_panel
```

```{r sez2-large-panel, fig.height=3.2}
p_large
```

## Layout combinato (4 pannelli — pronto per la presentazione)

```{r sez2-layout-completo, fig.height=7.5, fig.width=10}
layout_sezione2 <- (p_dgp) /
...
# Sezione 3 — Confronto selettori di $h$ e curva MISE

## Obiettivo

Nella terza sezione confrontiamo i valori di $h$ suggeriti da cinque metodi automatici con l'$h^*$ ottimale visualizzandoli anche graficamente sulla curva MISE empirica stimata nella prima sezione.

## Calcolo dei selettori

Per ogni selettore calcoliamo il **MISE empirico** valutato esattamente nel valore di $h$
suggerito — interpolando sulla griglia `h_seq` già simulata nella prima sezione.
...
Il grafico mostra il **MISE empirico** (stimato con gli stessi $B$ campioni della prima sezione)
...
```{r sez3-curva-ise, fig.height=4.5}
...
# Riusa mise_emp e h_seq già calcolati nella prima sezione
